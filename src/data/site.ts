...
export const FAQS: { q: string; a: string; href?: string; linkLabel?: string }[] = [
  ...
  {
    q: "断网后为什么没有自动触发？",
    a: "请检查设置 → 检测中是否启用了至少一种检测方式（默认启用 204 检测），并确认已启动监测；同时核对当前方案中的认证地址与校园网一致。",
  ...
  {
    q: "为什么新安装后程序不会自动检测？",
    a: "v5.0.0 起「启动后执行」默认为「无操作」：程序启动后停在待命状态，避免无谓的探测流量。在仪表盘右上角点击「启动检测」即可开始；希望开机即自动守护时，在 设置 → 系统 把「启动后执行」改为「开始检测」。",
    href: "/docs/automation/monitor",
    linkLabel: "查看断网自动重连",
  },
];
