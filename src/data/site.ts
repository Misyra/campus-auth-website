...
export const FEATURES = [
  {
    title: "断网重连",
    desc: "三重探针盯紧网络，断线或被劫持时秒级重连——宿舍熄灯、教学楼漫游也不掉线。",
    icon: "Wifi" as const,
  },
  {
    title: "多网络切换",
    desc: "按网关 IP 与 WiFi 自动选对方案与账号，跨校区来回跑也无需手动切换。",
...
export const FAQS: { q: string; a: string; href?: string; linkLabel?: string }[] = [
  {
    q: "断网后为什么没有自动触发？",
    a: "先看设置 → 网络监测里至少开一种检测（默认 204 开启）；再核对当前方案的认证地址与校园网是否一致。",
  ...
  },
  {
    ...
    href: "/docs?section=faq&item=troubleshoot",
    linkLabel: "查看排障指南",
  },
  {
  ...
  { q: "验证码总是识别失败？", a: "失败会自动重试；可调大等待、在任务里加截图断言，或在调试面板复现并导出反馈包。", href: "/docs?section=tasks&item=debug", linkLabel: "查看录制与调试" },
  { q: "Docker 怎么用？", a: "docker compose up -d --build 一键启动，数据落在命名卷持久化，开箱即带完整环境。", href: "/docs?section=getting-started&item=install", linkLabel: "查看安装与运行" },
  { q: "如何开机自启？", a: "在设置 → 系统 → 启动与运行中开启「开机自启动」，或执行 campus-auth --autostart enable。", href: "/docs?section=system&item=cli", linkLabel: "查看常用命令与文件说明" },
